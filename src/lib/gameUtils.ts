import { DiceRoll, DiceResult, DiceResultType, Unit, Effect, EffectType } from '@/types/game';
...
export function getEffectDescription(effectType: EffectType): string {
  const descriptions: Record<EffectType, string> = {
    ...
    [EffectType.BLOOD_DRUNK]: 'Unit must charge if possible. +1 to melee attacks.',
...
export function createEffect(
  ...
  const name = customName || type.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase());
...
export function getWoundStatus(unit: Unit): 'healthy' | 'wounded' | 'critical' | 'defeated' {
